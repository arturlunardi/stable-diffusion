...
class UNetResidualBlock(nn.Module):
    ...
    def forward(self, feature, time):
        # feature: [b_size, in_channels, height, width]
        # time: [1, 1280]
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        # [b_size, features, height, width] -> [b_size, features, height * 2, width * 2]
...
class UNetOutputLayer(nn.Module):
    ...
    def forward(self, x):
        # x: [b_size, 320, height/8, width/8]
        x = self.groupnorm(x)
        x = F.silu(x)

        # [b_size, 320, height/8, width/8] -> [b_size, 4, height/8, width/8]
        x = self.conv(x)
        return x
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.encoders = nn.ModuleList(
            [
                # [b_size, 4, height/8, width/8]
                SwitchSequential(nn.Conv2d(4, 320, kernel_size=3, padding=1)),
                # [b_size, 4, height/8, width/8] -> [b_size, 320, height/8, width/8]
                SwitchSequential(
                ...
                # [b_size, 320, height/8, width/8] -> [b_size, 320, height/16, width/16]
                SwitchSequential(
                    nn.Conv2d(320, 320, kernel_size=3, stride=2, padding=1)
                ),
                # [b_size, 320, height/8, width/8] -> [b_size, 640, height/16, width/16]
                SwitchSequential(
                ...
                # [b_size, 640, height/16, width/16] -> [b_size, 640, height/32, width/32]
                SwitchSequential(
                    nn.Conv2d(640, 640, kernel_size=3, stride=2, padding=1)
                ),
                # [b_size, 640, height/32, width/32] -> [b_size, 1280, height/32, width/32]
                SwitchSequential(
                ...
                # [b_size, 1280, height/32, width/32] -> [b_size, 1280, height/64, width/64]
                SwitchSequential(
        ...
        self.decoders = nn.ModuleList(
            [
                # [b_size, 2560, height/64, width/64] -> [b_size, 1280, height/64, width/64]
                SwitchSequential(UNetResidualBlock(2560, 1280)),
                # [b_size, 2560, height/64, width/64] -> [b_size, 1280, height/64, width/64]
                SwitchSequential(UNetResidualBlock(2560, 1280)),
                # [b_size, 2560, height/64, width/64] -> [b_size, 1280, height/64, width/64] -> [b_size, 1280, height/32, width/32]
                SwitchSequential(UNetResidualBlock(2560, 1280), Upsample(1280)),
                # [b_size, 2560, height/32, width/32] -> [b_size, 1280, height/32, width/32] -> [b_size, 1280, height/32, width/32]
                SwitchSequential(
                ...
                # [b_size, 1920, height/32, width/32] -> [b_size, 1280, height/32, width/32] -> [b_size, 1280, height/32, width/32] -> [b_size, 1280, height/16, width/16]
                SwitchSequential(
                    UNetResidualBlock(1920, 1280),
                    UNetAttentionBlock(8, 160),
                    Upsample(1280),
                ),
                # [b_size, 1920, height/16, width/16] -> [b_size, 640, height/16, width/16] -> [b_size, 640, height/16, width/16]
                SwitchSequential(
                    UNetResidualBlock(1920, 640), UNetAttentionBlock(8, 80)
                ),
                # [b_size, 1280, height/16, width/16] -> [b_size, 640, height/16, width/16] -> [b_size, 640, height/16, width/16]
                SwitchSequential(
                    UNetResidualBlock(1280, 640), UNetAttentionBlock(8, 80)
                ),
                # [b_size, 960, height/16, width/16] -> [b_size, 640, height/16, width/16] -> [b_size, 640, height/16, width/16] -> [b_size, 640, height/8, width/8]
                SwitchSequential(
                    UNetResidualBlock(960, 640),
                    UNetAttentionBlock(8, 80),
                    Upsample(640),
                ),
                # [b_size, 960, height/8, width/8] -> [b_size, 320, height/8, width/8] -> [b_size, 320, height/8, width/8]
                SwitchSequential(
                    UNetResidualBlock(960, 320), UNetAttentionBlock(8, 40)
                ),
                # [b_size, 640, height/8, width/8] -> [b_size, 320, height/8, width/8] -> [b_size, 320, height/8, width/8]
                SwitchSequential(
...
class Diffusion(nn.Module):
    ...
    def forward(self, latent: torch.Tensor, context: torch.Tensor, time: torch.Tensor):
        # latent: [b_size, 4, height/8, width/8]
        # context: [b_size, seq_len, n_embed]
        # time: [1, 320]
